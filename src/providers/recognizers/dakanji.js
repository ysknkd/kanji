...
import { kanjiDict, labelDict } from '../../labels/dakanji.js';

const MODEL_PATH = '/model/dakanji/model.json';
const INPUT_SIZE = 64;  // DaKanji accepts any size, but we normalize to 64x64
 ...
 * Supports ~6500 characters including kanji, hiragana, and katakana
 */
export const dakanjiProvider = {
  async loadModel() {
    if (model) return;
  ...
  preprocess(imageData, originalWidth, originalHeight) {
    return tf.tidy(() => {
      ...
      // DaKanji expects inverted grayscale (white strokes on black background)
